...
- [7. 시나리오](#7-시나리오)
  ...
  - [7.3 시나리오 테스트 (팝업)](#73-시나리오-테스트-팝업)
    - [7.3.1 시리리오 테스트](#731-시리리오-테스트)
    - [7.3.2 변수리스트](#732-변수리스트)
    - [7.3.3 Step 리스트](#733-step-리스트)
    - [7.3.4 Step 상세 내용](#734-step-상세-내용)
  - [7.4 aTworks Client 연동 (팝업)](#74-atworks-client-연동-팝업)
  - [7.5 Case 등록](#75-case-등록)
  - [7.6 변수 등록](#76-변수-등록)
  - [7.7 Step 등록](#77-step-등록)
    - [7.7.1 Login API](#771-login-api)
    - [7.7.2 API 검색](#772-api-검색)
    - [7.7.3 Query 등록](#773-query-등록)
    - [7.7.4 Script 소개](#774-script-소개)
    - [7.7.5 Script 복사하기](#775-script-복사하기)
    - [7.7.6 Script 비교하기](#776-script-비교하기)
    - [7.7.7 Script 기다리기](#777-script-기다리기)
  - [7.8 Script Item Type](#78-script-item-type)
    - [7.8.1 Step](#781-step)
    - [7.8.2 변수](#782-변수)
    - [7.8.3 Text](#783-text)
    - [7.8.4 Number](#784-number)
    - [7.8.5 난수](#785-난수)
  - [7.9 화면 녹화](#79-화면-녹화)
    - [7.9.1 aTworks Client 실행여부 확인](#791-atworks-client-실행여부-확인)
    - [7.9.2 UI 화면 수행](#792-ui-화면-수행)
    - [7.9.3 녹화 시작](#793-녹화-시작)
    - [7.9.3 녹화 정지](#793-녹화-정지)
    - [7.9.4 케이스 선택 팝업](#794-케이스-선택-팝업)
    - [7.10 테스트 이력](#710-테스트-이력)
- [8. 점검](#8-점검)
...
- `열기`버튼: [aTworks client 프로그램과 연동하는 팝업](#74-atworks-client-연동-팝업)을 오픈한다
...
- `+신규 변수`버튼: [신규 변수 등록 팝업](#76-변수-등록)을 오픈한다
...
- `+ 신규 스탭`: [신규 스탭을 추가하는 팝업](#75-case-등록)을 오픈한다
...
### 7.3 시나리오 테스트 (팝업)
> 선택한 시나리오 수행하는 팝업으로 실시간으로 스텝들이 수행되는 것을 확인 할 수 있습니다. 
> 시나리오 테스트, 변수 리스트, Step 리스트, Step 상세 내용의 4가지 단락으로 구분됩니다.

![시나리오 테스트](./images/7.3_시나리오_테스트.png)

#### 7.3.1 시리리오 테스트

![시나리오 테스트](./images/7.3.1_시나리오_테스트.png)
![시나리오 테스트2](./images/7.3.1_시나리오_테스트2.png)
...
#### 7.3.2 변수리스트
> 시나리오에 등록된 변수를 보여주며, 시나리오 수행에 따라 값을 참조하거나 변경할 수 있는 데이터입니다.   
> 시나리오 테스트 수행전에 직접 값을 변경해서 수행할 수도 있습니다. 

![변수리스트](./images/7.3.2_변수_리스트.png)



#### 7.3.3 Step 리스트
> 시나리오에 등록된 Step을 순차적으로 나열한 리스트로 각각 Item을 선택하면 [Step 상세 내용](#734-step-상세-내용)에 데이터가 변경됩니다.

![Step 리스트](./images/7.3.3_Step_리스트.png)

- Step 유형 (Login, Script, API, Query)
- Case 이름과 Step 이름 2줄로 표현하여 보여준다
- Step 테스트 결과 

#### 7.3.4 Step 상세 내용
> 선택한 Step에 상세 정보를 보여주는 화면으로 Step 유형에 따라 보여지는 화면이 다릅니다. 

![Step 상세 내용](./images/7.3.4_Step_상세내용.png)


### 7.4 aTworks Client 연동 (팝업)
> 화면녹화를 위해 aTworks Client 와 web-socket 연결을 위한 팝업으로 미리 등록한 UI 서버가 존재해야 합니다. 

![aTworks Client 연동](./images/7.4_aTworks_client_연동.png)

- Client 연결상태: aTworks Client 연결유무 확인
  (우측 팝업처럼 미실행 상태인 경우 aTworks client 수행후 갱신 버튼 클릭)
- Chrome 버전: 사용중이 PC에 설치된 Chrome 버전
- 드라이버 유무: 화면녹화를 위해 설치가 되어야 하는 프로그램으로 aTworks Client 수행시 자동으로 다운로드 한다  
(드라이버가 최신버전인 경우 설치가 안되는 경우 관리자에 문의해야 한다)
- Test UI Server: 미리 [UI 서버관리](#313-ui-서버-등록)를 통해 등록해야 한다
- `열기`버튼: 선택시에 새로운 윈도우 창으로 선택된 UI 화면이 나타난다

### 7.5 Case 등록
> 선택한 시나리오에 포함되는 신규 Case를 등록하는 팝업입니다. 

![Case 등록](./images/7.5_Case_등록.png)

- Case 이름
- Case 설명

### 7.6 변수 등록
> 선택한 시나리오에서 사용하는 변수를 등록하는 팝업입니다. 

![변수 등록](./images/7.7_변수_등록.png)

- 변수유형: String/Number 중 택1
- 변수명: 구분가능한 식별자
- 초기값: 초기데이터

### 7.7 Step 등록
> Step은 실제 테스트가 수행되는 단위로 크게 Login, API 호출, Query, Script로 구분할 수 있습니다.
 
![Step 등록](./images/7.7_Step_등록.png)

#### 7.7.1 Login API
> 로그인 API 세션 연동을 위한 프로그램으로 Login을 관리하는 시스템에 반드시 기본적으로 들어가야 합니다. 

![Login API](./images/7.7.1_Login_API.png)

- 로그인 정보: 
  - 디폴트 로그인: 미리 지정된 로그인 정보로 로그인을 시도한다
  - 직접 입력: 아이디와 비밀번호를 직접 입력하여 로그인을 시도한다
- 아이디: 직접 입력시만 사용
- 비밀번호: 직접 입력시만 사용
- Error 발생시 계속진행: Step이 실패하는 경우 후속 Step 진행 여부 선택

#### 7.7.2 API 검색
> 기존에 등록된 API에서 검색하는 기능으로 Request 정보를 알고 있는 경우 직접 등록이 가능합니다. 

![API 검색](./images/7.7.2_API_검색.png)

- API 검색 
  - `검색`버튼: 좌측에 입력된 검색어로 API를 검색한다
  - 리스트: Http Method와 API 정보가 검색된다
  - 선택하기: `->` 버튼을 클릭시 `선택한 API` 내용이 변경된다
- 선택한 API 
  - Http method와 URL 경로를 보여준다
  - Error 발생시 계속진행: Step이 실패하는 경우 후속 Step 진행 여부 선택
  - Request Body: 등록된 API에 Request 정보를 확인하여 변경할 수 있다
  
#### 7.7.3 Query 등록
> 시스템 Database 접근하여 직접 쿼리를 수행하고 결과를 JSON 데이터로 입력받아서 후속 Script에서 재사용이 가능합니다.   
> 사이트에 따라서 적용이 안될 수도 있습니다. 

![Query 등록](./images/7.7.3_Query_등록.png)
- 제목: 식별가능한 구분자
- 테스트서버: Database 서버 (사전 등록 필수)
- `Test`버튼: Query 내용을 미리 확인해볼 수 있다
- Error 발생시 계속진행: Step이 실패하는 경우 후속 Step 진행 여부 선택
- Request Body: SQL Query 작성
- Response Body: SQL 수행 결과를 JSON 데이터로 출력한다
- delete, update, create, drop 명령어 사용금지


#### 7.7.4 Script 소개
> API, Query 등을 서로 연계하기 위해 총 3가지 기능을 제공합니다. 

| Script 유형| 설명|
| --| --|
| 복사하기| From 에서 추출된 데이터를 To에서 지정된 데이터에 Copy|
| 비교하기| A/B 데이터를 비교하여 True/False에 따라 후속 프로세스에 대한 처리여부를 결정|
| 복사하기| 최대 3000ms 까지 대기후 계속 수행|
 

#### 7.7.5 Script 복사하기
> text, number, 난수, 변수, 상위 step의 response data 값을 copy 하여 변수 또는 하위 step의 request data 값으로 이동합니다. 
 
![Script 복사하기](./images/7.7.5_복사하기.png)
- Script 공통속성
  - Script 유형
  - 제목: 식별가능한 구분자
  - Error 발생시 계속진행: Step이 실패하는 경우 후속 Step 진행 여부 선택
- FROM DATA: 추출할 값이 있는 대상
- TO DATA: FROM DATA에서 추출된 item을 입력할 대상
- 각 ITEM 유형 

| 이름| 유형| 설명| step| text| number| 난수| 변수|
|---|---|---|---|---|---|---|---|
| FROM DATA| 복사하기| 복사를 수행할 item| O| O| O| O| O|
| TO DATA| 복사하기| 복사가 수행될 item| O| X| X| X| O|
| ITEM| 비교하기| 비교 대상 ITEM A, B| O| O| O| X| O|


#### 7.7.6 Script 비교하기
> text, 변수, step의 response data 값을 비교하여 후속 프로세스를 정하게 됩니다.   

![Script 비교하기](./images/7.7.6_비교하기.png)

- 데이터 A: 비교대상 A
- 수식: (==, !=, <, <=, is null) 중 택 1
- 데이터 B: 비교대상 B
- 후속 프로세스 
  - 시나리오 테스트 성공: 후속 프로세스를 수행하지 않고 시나리오 성공으로 처리 
  - 시나리오 테스트 실패: 후속 프로세스를 수행하지 않고 시나리오 실패로 처리
  - 하위 Step으로 이동: 지정한 하위 Step으로 프로세스가 이동되고 사이의 Step의 테스트 대상에서 제외

#### 7.7.7 Script 기다리기
> 최대 3000ms 까지 대기후 계속 수행합니다.

### 7.8 Script Item Type

#### 7.8.1 Step
> API, Query 수행시 사용되는 request, response data

![Step](./images/7.8.1_Step.png)

- 데이터 유형: Step
- Case 선택
- Step 선택
- 데이터: Javascript Json 객체 조회하는 방식으로 조회할 수 있다 (단 List의 경우 .[index] 입력 필요)
- Request or Response Data
  FROM의 경우 Response Data가 TO의 경우 Request Data가 자동으로 보여진다

 
#### 7.8.2 변수
> 사전에 등록한 변수의 데이터를 활용한다 [변수](#76-변수-등록)

![변수](./images/7.8.2_변수.png)

#### 7.8.3 Text
> 사용자 직접 입력하는 String Data

#### 7.8.4 Number
> 사용자 직접 입력하는 Number Data 
#### 7.8.5 난수
> 사전에 등록한 난수의 데이터를 활용한다 [난수](#4-난수)
![난수](./images/7.8.5_난수.png)
- 난수 스타일: D_DAY, LIST, Date 등
- 난수 선택


### 7.9 화면 녹화
> UI 화면에서 발생한 API Log를 조회하여 테스트 케이스로 변환하는 기능을 제공합니다.   
> 이를 위해서는 aTworks Client.exe 가 설치되어 있어야 하며 이는 설치파일을 별도로 제공합니다.

#### 7.9.1 aTworks Client 실행여부 확인
- 1. Window 실행 프로그램에 `aT`라고 적힌 파란색 트레이 아이콘이 있으면 Client 모듈이 실행중이다

![aTworks Client 실행여부](./images/7.9.1_aTworks_Client_실행여부.png)

- 2. 시나리오 수행 화면에서 aTworks Client 연동 팝업 호출시 연결상태를 확인할 수 있다  

![aTworks Client 연동](./images/7.4_aTworks_client_연동.png)


#### 7.9.2 UI 화면 수행 
> aTworks Client 연동 팝업에서 `열기`버튼 클릭시 현재창과 별도로 등록된 UI 화면이 새창으로 열리게 됩니다. 

![UI 화면 수행](./images/7.9.2_UI_화면_수행.png)


#### 7.9.3 녹화 시작
> 시나리오 수행 화면에서 `녹화`버튼을 클릭시 화면 녹화가 시작됩니다.  
> 별도로 열린 UI 화면에서 테스트를 수행합니다. 

![녹화 시작](.imaages/../images/7.9.3_녹화_시작.png)

- `초기화`버튼: 현재까지 녹화된 API 이력을 초기화 합니다.


#### 7.9.3 녹화 정지
> 화면 테스트를 완료한 후 시나리오 수행 화면에서 `정지`버튼을 선택하면 화면에서 
> 발생한 API 테스트 이력을 조회후 `케이스 선택` 팝업이 오픈됩니다.

![녹화 정지](./images/7.9.4_녹화_정지.png)


#### 7.9.4 케이스 선택 팝업
> 팝업에서 등록할 Case를 선택하고 등록 버튼을 클릭합니다.  
> 선택한 케이스에 화면에서 테스트 API 이력이 조회되는 것을 확인 할 수 있습니다.

![케이스 선택 팝업](./images/7.9.4_케이스_선택_팝업.png)
![케이스 선택 팝업2](./images/7.9.4_케이스_선택_팝업2.png)
#### 7.10 테스트 이력
> 시나리오 테스트 이력 리스트 조회합니다.

![테스트 이력](./images/7.10._테스트_이력.png)
- 검색조건
  - 시나리오 이름
  - 관리자
  - 테스트 결과 (성공/실패)
...
- 테스트 이력 리스트
  ...
  - 테스트 시간
  - 소요시간(초)
  - 상세보기: 리스트의 🔍 버튼 또는 리스트 행을 더블 클릭 하면 [시나리오 테스트 상세 결과](#73-시나리오-테스트-팝업)를 확인할 수 있다 
